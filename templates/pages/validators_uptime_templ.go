...
func meetsPoWSla(report *db.SlaNodeReport, blockQuota int32) bool {
	if blockQuota <= 0 {
...
func meetsPoSSla(report *db.SlaNodeReport) bool {
...
func meetsSlaThreshold(report *db.SlaNodeReport, blockQuota int32) bool {
...
func getUptimeColor(report *db.SlaNodeReport, blockQuota int32) string {
	if report.NumBlocksProposed == 0 {
		return "bg-gray-800"
	}
...
func getStatusText(validatorStatus string, report *db.SlaNodeReport, blockQuota int32) (string, string) {
...
func getValidatorStatusDisplay(validatorInfo *ValidatorUptimeInfo, rollupData *db.SlaRollup) (string, string) {
	var blockQuota int32 = 0
	if rollupData != nil {
		blockQuota = rollupData.BlockQuota
	}

	var report *db.SlaNodeReport = nil
...
func getValidatorStatusText(validatorInfo *ValidatorUptimeInfo, rollupData *db.SlaRollup) string {
...
func getValidatorStatusClass(validatorInfo *ValidatorUptimeInfo, rollupData *db.SlaRollup) string {
	switch validatorInfo.Validator.Status {
	case "active":
...
func getSlaStatusText(validatorInfo *ValidatorUptimeInfo, rollupData *db.SlaRollup) string {
...
func getSlaStatusClass(validatorInfo *ValidatorUptimeInfo, rollupData *db.SlaRollup) string {
	if validatorInfo.Validator.Status != "active" {
		return "" // Only show SLA for active validators
	}
...
func getLatestRollupInfo(validators []*ValidatorUptimeInfo) *db.SlaNodeReport {
	if len(validators) == 0 {
		return nil
	}
...
func getRollupInfo(validators []*ValidatorUptimeInfo, rollupId int32) *db.SlaNodeReport {
	if len(validators) == 0 {
		return nil
	}
